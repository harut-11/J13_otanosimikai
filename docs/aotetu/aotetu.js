...
function movePlayer(dice) {
  ...
  const interval = setInterval(() => {
    ...
    if (steps >= dice) {
      ...
      if (p.pos === TILE_COUNT - 1) {
        addLog(`🚩 <span style="color:${p.color}">${p.name}</span> がゴールにピッタリ到着！`);
        ...
        // 現在の年数を計算（0年目から始まるので +1）
        const currentYear = Math.floor(totalTurns / (players.length * 12)) + 1;

        // 【修正ポイント】現在の年数が設定された最大年数(maxYears)に達しているかチェック
        if (currentYear >= maxYears) {
          setTimeout(() => { 
            alert(`${p.name} が最終ゴール！結果発表に移ります。`); 
            showFinalResults(); 
          }, 500);
        } else {
          // 2年目以降がある場合の更新処理
          setTimeout(() => {
            alert(`${p.name} がゴール！次の年へ進みます。`);
            ...
            // ターンと累計ターン数を「次の年の最初」まで進める
            turnIdx = 0;
            totalTurns = currentYear * players.length * 12; 
            
            addLog(`🚀 ${currentYear + 1}年目スタート！`);
            addLog(`😈 貧乏神は最後尾の <span style="color:${lastPlayer.color}">${lastPlayer.name}</span> に憑依しました。`);
